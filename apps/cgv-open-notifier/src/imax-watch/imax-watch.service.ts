...
import { filterImax } from '../cgv/utils/theater-type-filter';
...
const TIMEOUT_NAME = 'imax-watch-cycle';
const ERROR_NOTIFY_THRESHOLD = 5; // 연속 사이클 오류 알림 임계치

/*
 * 날짜 상관없이 용산 IMAX의 신규 오픈/회차 추가를 감지하는 감시자.
 * 서버 기동 시 자동으로 시작되어, 사이클마다 horizonDays 만큼의 날짜를 조회해
 * 이전 스냅샷과 diff한다. 사이클 사이 대기 시간은 설정값 ±5초 랜덤.
 */
@Injectable()
export class ImaxWatchService implements OnApplicationBootstrap {
  private readonly logger = new Logger(ImaxWatchService.name);
  ...
  constructor(
    ...
    this.snapshotPath = resolve(
      process.cwd(),
      this.configService.getOrThrow<string>('IMAX_SNAPSHOT_PATH'),
    );
  }
      ...
      .catch((err: unknown) => {
        ...
        if (this.consecutiveErrors >= ERROR_NOTIFY_THRESHOLD) {
          void this.telegramService.sendMessage(
            'IMAX 감시 중 오류가 반복되고 있습니다. 감시는 계속됩니다.',
          );
          this.consecutiveErrors = 0;
        }
      })
  ...
  private async runCycle(): Promise<void> {
    ...
    for (let i = 0; i < dates.length; i++) {
      ...
      if (screenings) {
        ...
        this.logger.log(
          `(${i + 1}/${dates.length}) ${date} IMAX ${screenings.length}건`,
        );
      }
